function initDesktop(doc) {
  d(() => {
    ...
    cls(fieldset, "absolute right-[1rem]");
    const fieldContent = document.createElement("div");
    fieldContent.innerHTML = `
    <button title="Voir la légende" id="replier" onclick="replierLegende()" class="mb-2 !rounded-2xl !bg-black !text-white !font-bold px-1 text-center cursor-pointer !border-1 !border-solid"> \u2B9C </button>
...
function replierLegende() {
  ...
  if (legende.classList.contains("hidden")) {
    legende.classList.remove("hidden");
    legende.classList.add("flex");
    table.style.width = "90%";
    buttonLegende.textContent = "\u2B9F";
    buttonLegende.title = "Cacher la légende";
  } else {
    legende.classList.remove("flex");
    table.style.width = "95%";
    legende.classList.add("hidden");
    buttonLegende.textContent = "\u2B9C";
    buttonLegende.title = "Voir la légende";
  }
}
