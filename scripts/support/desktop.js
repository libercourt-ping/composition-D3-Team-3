function initDesktop(doc) {
  d(() => {
    ...
    h2.textContent = "Saison 2026-2027, phase 1";
    ...
    cls(messages, "w-full px-4 max-h-[20rem] overflow-auto mb-3");
...
function init() {
  d(() => {
    ...
    for (let j in joueurs) {
      ...
      const bgClazz =
        player.teammate && player.hasRealPlayers ? bgClazzes[index] : "bg-none";
      cls(
        divPoints,
    ...
    for (let i = 0; i < matchs.length; ++i) {
      ...
      for (let k = 0; k < allJoueurs.length; ++k) {
        ...
        if (!joueur.hasRealPlayers) {
          td.classList.add("_renfort");
        }
        td.classList.add(joueur.teammate ? "_teammate" : "_renfort");
